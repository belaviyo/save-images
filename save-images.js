...
var download = (() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      fetch(obj.src).then(response => {
        if (response.ok) {
          ...
          return response.blob().then(blob => zip.file(name, blob)).then(resolve);
        }
        else {
  ...
  function one() {
    ...
    if (j1) {
      Promise.all([
        j1 ? download(j1).catch(() => {}) : Promise.resolve(),
        j2 ? download(j2).catch(() => {}) : Promise.resolve(),
        j3 ? download(j3).catch(() => {}) : Promise.resolve(),
        j4 ? download(j4).catch(() => {}) : Promise.resolve(),
        j5 ? download(j5).catch(() => {}) : Promise.resolve()
      ]).then(one);
    }
    else {
